...
export function ContactForm({ contactEmail }: { contactEmail?: string }) {
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    <Button type="submit" size="lg" disabled={!contactEmail}><Send className="h-4 w-4 rtl-flip" />إرسال الرسالة</Button>
    {!contactEmail ? <p className="text-body-sm text-warning-500">قناة البريد قيد التجهيز، وستتاح قريبًا.</p> : null}
  </form>;
}
