...
const typeLabels: Record<string, string> = { job: "وظيفة", volunteering: "تطوع", co_op: "تدريب تعاوني" };
...
export function AdminAnalytics({ trend, opportunityTypes, applicationStatuses, approved, totalOrganizations }: { trend: Array<{ date: string; value: number }>; opportunityTypes: Array<{ name: string; value: number }>; applicationStatuses: Array<{ name: string; value: number }>; approved: number; totalOrganizations: number }) {
  ...
  return <div className="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-2">
    <ChartCard title="اتجاه الطلبات" action={<div className="flex rounded-md bg-neutral-100 p-1">{([7, 30] as const).map((value) => <button key={value} onClick={() => setPeriod(value)} className={`rounded-sm px-3 py-1 text-caption ${period === value ? "bg-surface text-primary-700 shadow-xs" : "text-neutral-500"}`}>{value} أيام</button>)}</div>}>
      {visibleTrend.some((item) => item.value) ? <ResponsiveContainer width="100%" height={260}><AreaChart data={visibleTrend}><defs><linearGradient id="trendFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--color-primary-500)" stopOpacity={0.32}/><stop offset="100%" stopColor="var(--color-primary-500)" stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke="var(--border)" vertical={false}/><XAxis dataKey="date" tickFormatter={formatDateArabic} tick={{ fill: "var(--text-secondary)", fontSize: 11 }} tickLine={false} axisLine={false}/><YAxis allowDecimals={false} tick={{ fill: "var(--text-secondary)", fontSize: 11 }} tickLine={false} axisLine={false}/><Tooltip contentStyle={tooltipStyle} labelFormatter={(value) => formatDateArabic(String(value))}/><Area type="monotone" dataKey="value" name="الطلبات" stroke="var(--color-primary-600)" strokeWidth={3} fill="url(#trendFill)" activeDot={{ r: 5 }}/></AreaChart></ResponsiveContainer> : <EmptyChart />}
    </ChartCard>
    <ChartCard title="الفٌرص حسب النوع"><ResponsiveContainer width="100%" height={260}><PieChart><Pie data={opportunityTypes.map((item) => ({ ...item, label: typeLabels[item.name] ?? item.name }))} dataKey="value" nameKey="label" innerRadius={62} outerRadius={92} paddingAngle={3}>{opportunityTypes.map((_, index) => <Cell key={index} fill={pieColors[index % pieColors.length]} />)}</Pie><Tooltip contentStyle={tooltipStyle}/></PieChart></ResponsiveContainer></ChartCard>
    <ChartCard title="حالات الطلبات"><ResponsiveContainer width="100%" height={260}><BarChart data={applicationStatuses.map((item) => ({ ...item, label: statusLabels[item.name] ?? item.name }))} layout="vertical" margin={{ right: 10, left: 10 }}><CartesianGrid stroke="var(--border)" horizontal={false}/><XAxis type="number" allowDecimals={false} tick={{ fill: "var(--text-secondary)", fontSize: 11 }} axisLine={false}/><YAxis type="category" dataKey="label" width={76} tick={{ fill: "var(--text-secondary)", fontSize: 11 }} axisLine={false} tickLine={false}/><Tooltip contentStyle={tooltipStyle}/><Bar dataKey="value" name="الطلبات" radius={[4, 4, 4, 4]}>{applicationStatuses.map((item) => <Cell key={item.name} fill={statusColors[item.name] ?? "var(--color-neutral-400)"}/>)}</Bar></BarChart></ResponsiveContainer></ChartCard>
