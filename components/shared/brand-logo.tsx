...
export function BrandLogo({ className, inverted = false }: { className?: string; inverted?: boolean }) {
  return <Link href="/" className={cn("group inline-flex items-center gap-2.5", className)} aria-label="فٌرص - الرئيسية">
    ...
    <span className={cn("font-heading text-2xl font-bold", inverted ? "text-white" : "text-neutral-900")}>فٌرص</span>
  </Link>;
}
