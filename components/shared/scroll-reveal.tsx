...
const hiddenVariants: Record<RevealVariant, string> = {
  ...
  fade: "opacity-0",
  start: "translate-x-14 opacity-0 rtl:-translate-x-14",
  end: "-translate-x-14 opacity-0 rtl:translate-x-14",
...
export function ScrollReveal({ children, className, delay = 0, variant = "up" }: { children: React.ReactNode; className?: string; delay?: number; variant?: RevealVariant }) {
  ...
  return <div ref={ref} style={{ transitionDelay: `${delay}ms`, transitionTimingFunction: "cubic-bezier(.22, 1, .36, 1)" }} className={cn("transition-[opacity,transform] duration-1000 motion-reduce:translate-x-0 motion-reduce:translate-y-0 motion-reduce:scale-100 motion-reduce:opacity-100", hiddenVariants[variant], visible && "translate-x-0 translate-y-0 scale-100 opacity-100", className)}>{children}</div>;
}
