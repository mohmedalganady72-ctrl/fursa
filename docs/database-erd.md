...
- **`applications` موحّد** بنفس المنطق لحقول التقديم المختلفة حسب نوع الفٌرصة. عمود `compatibility_breakdown` يحفظ JSON لتفصيل كل معيار ووزنه وقت التقييم تحديدًا، وليس الأوزان الحالية في النظام — هذا يضمن أن شرح الدرجة لأي تقديم قديم يبقى دقيقًا حتى لو تغيّرت الأوزان الافتراضية لاحقًا.
...
- **`reports`**: نفس منطق `target_id`/`target_type` المرن أعلاه، يدعم التبليغ عن رسالة أو مستخدم أو فٌرصة بجدول واحد.
